...
- Extract framework-agnostic core into `@valent/docx-editor-core` with zero React dependencies
- Keep `@valent/docx-editor-react` as the React UI package (preserves npm stats, no migration for existing users)
- Use Bun workspaces for monorepo management (already using Bun as runtime)
- Enable community Vue/Svelte wrappers that depend only on `@valent/docx-editor-core`
...
**Decision:** Split into exactly `@valent/docx-editor-core` and `@valent/docx-editor-react` (React).
...
**Decision:** `@valent/docx-editor-react` stays as the React package name. No rename.
...
**React (`@valent/docx-editor-react`):**
...
**Decision:** React package imports from `@valent/docx-editor-core` (the npm package name), not relative paths.
...
**Core interface (`@valent/docx-editor-core`):**
...
**React adapter (`@valent/docx-editor-react`):**
...
**Vue adapter (`@valent/docx-editor-vue`):**
...
- `EditorPluginCore`, `PanelConfig`, `PluginPanelProps` (without ReactNode), `RenderedDomContext` → `@valent/docx-editor-core`
...
**[Risk] Internal import churn** → Files that currently do `../docx/parser` will need `@valent/docx-editor-core` imports. Mitigation: batch find-and-replace, verify with typecheck.
