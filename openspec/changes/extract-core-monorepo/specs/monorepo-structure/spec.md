...
The `packages/core/` directory SHALL contain all framework-agnostic source code with its own `package.json` named `@sqren/docx-editor-core`.
...
The `packages/react/` directory SHALL contain all React-dependent source code with its own `package.json` named `@sqren/docx-editor-react`.
...
- **THEN** `@sqren/docx-editor-core` SHALL be listed in dependencies
...
- **AND** it SHALL externalize `@sqren/docx-editor-core` (not bundle it)
...
- **THEN** tests SHALL import from both `@sqren/docx-editor-core` and `@sqren/docx-editor-react`
...
- **AND** the app SHALL render a functional DOCX editor using `@sqren/docx-editor-vue` and `@sqren/docx-editor-core`
...
The `packages/vue/` directory SHALL contain an empty scaffolded package named `@sqren/docx-editor-vue` with a dependency on `@sqren/docx-editor-core`, ready for community contribution.

#### Scenario: Vue package has minimal package.json

- **WHEN** inspecting `packages/vue/package.json`
- **THEN** it SHALL have name `@sqren/docx-editor-vue`
- **AND** `@sqren/docx-editor-core` SHALL be listed in dependencies
