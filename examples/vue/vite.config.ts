...
// resolve `@sqren/docx-editor-vue` + `/agents/*` through node_modules
// (the workspace's published dist/) so the deployment shows the real
// consumer experience. Core deep paths (e.g.
// `@sqren/docx-editor-core/headless`) are kept aliased to source in
...
const coreAliases = [
  ...
  // Exact match for bare @sqren/docx-editor-core (must come AFTER prefix match)
  {
    find: /^@sqren\/docx-editor-core$/,
