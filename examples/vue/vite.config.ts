...
// resolve `@valent/docx-editor-vue` + `/agents/*` through node_modules
// (the workspace's published dist/) so the deployment shows the real
// consumer experience. Core deep paths (e.g.
// `@valent/docx-editor-core/headless`) are kept aliased to source in
...
const coreAliases = [
  {
    find: '@valent/docx-editor-core/headless',
    replacement: path.join(monorepoRoot, 'packages/core/src/headless.ts'),
  },
  {
    find: '@valent/docx-editor-core/core-plugins',
  ...
  resolve: {
    alias: usePublished
      ...
      : [
          ...
          {
            find: '@valent/docx-editor-vue/styles.css',
