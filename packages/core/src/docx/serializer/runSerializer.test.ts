...
// the shape is silently dropped (e.g. footer "Classification - Internal"
// banners, SOW title boxes).
          ...
          content: [
            {
              type: 'paragraph',
              content: [
                { type: 'run', content: [{ type: 'text', text: 'Classification - Internal' }] },
              ],
            },
          ],
        },
...
describe('shape text body is gated on text presence, not geometry', () => {
  test('rect shape with non-empty text body serializes a w:txbxContent with the run text', () => {
    ...
    expect(xml).toContain('Classification - Internal');
    expect(xml).toContain('</w:txbxContent>');
