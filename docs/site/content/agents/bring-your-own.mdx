...
description: 'Wire the DOCX tools into Vercel AI SDK, OpenAI, Anthropic, LangChain, or any runtime that accepts OpenAI-style function schemas, with one shared executor.'
category: 'Agents'
order: 52
---
...
```ts
import { getToolSchemas } from '@valent/docx-editor-agents';
...
import { getToolSchemas, executeToolCall } from '@valent/docx-editor-agents';
...
```ts
import { McpServer } from '@valent/docx-editor-agents/mcp';
...
```ts
import { agentTools, executeToolCall, createReviewerBridge } from '@valent/docx-editor-agents';
