...
`createReviewerBridge(reviewer)` wraps the reviewer in the same `EditorBridge` interface the live editor exposes. That means the [agent tools](/docs/1.x/agents/tools), `executeToolCall`, and the [MCP server](/docs/1.x/agents/mcp) all work against a static file:

```ts
import { DocxReviewer, createReviewerBridge, executeToolCall } from '@valent/docx-editor-agents';
