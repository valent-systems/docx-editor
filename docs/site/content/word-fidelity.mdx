...
- Unmodeled XML elements and attributes are kept and re-emitted in place. (One exception: legacy run-level VML drawn shapes other than watermarks — `v:rect`, `v:line`, non-text `v:shape` — are not yet modeled or preserved.)
