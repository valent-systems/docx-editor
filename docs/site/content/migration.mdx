...
description: "Move from @valent/docx-js-editor 0.x to the 1.x package set. Package-rename map, moved-symbol map, i18n restructure, license update."
...
0.x was a single package. 1.x splits it into five npm packages by concern (plus `@valent/nuxt-docx-editor`, a Nuxt module wrapping the Vue adapter):

| What you used in 0.x | Where it is in 1.x |
|---|---|
| The `<DocxEditor>` component, toolbar, dialogs, hooks, plugin host | `@valent/docx-editor-react` (or `@valent/docx-editor-vue`) |
| The document model and OOXML parse/serialize (`Document`, `Paragraph`, `createEmptyDocument`, ...) | `@valent/docx-editor-core` |
| Anything AI: the reviewer, agent UI components, the MCP server | `@valent/docx-editor-agents` |
| Locale strings (`pl`, `de`, `pt-BR`, ...) | `@valent/docx-editor-i18n` |

In practice a React app still imports from one package, `@valent/docx-editor-react`, which pulls in `-core` and `-i18n` for you. You only name the other packages when you import the document model, the agent toolkit, or locale data directly. The rest of this guide is the symbol-level detail behind that split.
...
3. Change every remaining `@valent/docx-js-editor` import to `@valent/docx-editor-react`.
...
The monolithic `@valent/docx-js-editor` split into five packages under the `@valent/docx-editor-*` namespace, plus the `@valent/nuxt-docx-editor` module.

| Old (0.x) | New (1.x) |
|---|---|
| `@valent/docx-js-editor` | `@valent/docx-editor-react` |
| `@valent/docx-js-editor/core` | `@valent/docx-editor-core` |
| `@valent/docx-js-editor/mcp` | `@valent/docx-editor-agents/mcp` |
| `@valent/docx-js-editor/i18n/<code>.json` | `@valent/docx-editor-i18n/<code>` (subpath) or named export off the root |
| `@valent/docx-editor-agents` (AGPL-3.0) | `@valent/docx-editor-agents` (Apache 2.0) |

Install the package set you actually use:

```bash
# Bare minimum for a React editor:
npm uninstall @valent/docx-js-editor
...
Five categories of symbols moved across packages (not just subpaths). A plain package-name swap to `@valent/docx-editor-react` won't resolve these. **If a symbol from this list lives in your code, point its import at the new home below; otherwise every one of them becomes a "no exported member" error.**

| Symbol(s) | Was (0.x) | Now (1.x, recommended) |
|---|---|---|
| `createEmptyDocument`, `createDocumentWithText`, `CreateEmptyDocumentOptions` | `@valent/docx-js-editor` | `@valent/docx-editor-react` or `@valent/docx-editor-vue` (re-exported in 1.0.1 from `-core`) |
| Document-tree types most apps reach for: `Document`, `Comment`, `Paragraph`, `Run`, `Insertion`, `Deletion`, `TrackedChangeInfo`, `Hyperlink`, `Table` | `@valent/docx-js-editor` | `@valent/docx-editor-core` (root entry surfaces them in 1.0.1) |
| Rarer document-tree types: `Image`, `Footnote`, `Endnote`, `TableRow`, `TableCell` | `@valent/docx-js-editor` | `@valent/docx-editor-core` (root entry; also on `/headless`) |
| Agent UI components: `AgentPanel`, `AgentChatLog`, `AgentComposer`, `AgentSuggestionChip`, `AgentTimeline` (and their `*Props` types) | `@valent/docx-js-editor` | `@valent/docx-editor-agents/react` (or `/vue`) |
| `PluginHost`, `PluginHostProps`, `PluginHostRef`, `templatePlugin` | `@valent/docx-js-editor` | `@valent/docx-editor-react/plugin-api` |
...
Once the moved symbols are handled, every remaining `@valent/docx-js-editor` import changes to `@valent/docx-editor-react`. Only the package name changes; the imported names stay the same.
...
| `@valent/docx-js-editor/core` | `@valent/docx-editor-core` |
| `@valent/docx-js-editor/mcp` | `@valent/docx-editor-agents/mcp` |
| `@valent/docx-js-editor/i18n/<code>.json` | `@valent/docx-editor-i18n` (see [i18n restructure](#i18n-restructure) below) |
...
```diff
- import { FormattingBar } from '@valent/docx-js-editor';
+ import { Toolbar } from '@valent/docx-editor-react/ui';
```
...
```diff
- import { Toolbar } from '@valent/docx-js-editor';
+ import { EditorToolbar } from '@valent/docx-editor-react/ui';
// or compose your own:
+ import { Toolbar, ToolbarButton, ToolbarGroup } from '@valent/docx-editor-react/ui';
```
...
import { pl, ptBR } from '@valent/docx-editor-i18n';
```

The diff from 0.x:

```diff
- import pl from '@valent/docx-js-editor/i18n/pl.json';
+ import pl from '@valent/docx-editor-i18n/pl';
```
...
```diff
- import(`@valent/docx-js-editor/i18n/${locale}.json`).then(setI18n);
+ import(`@valent/docx-editor-i18n/${locale}`).then((m) => setI18n(m.default));
```
...
+ import type { LocaleStrings, PartialLocaleStrings } from '@valent/docx-editor-i18n';
+ type LocaleI18n = LocaleStrings | PartialLocaleStrings | undefined;
```

## License change

The 0.x line of `@valent/docx-editor-agents` was AGPL-3.0. In 1.0, every published package is **Apache 2.0**. If you tracked license obligations for compliance review, update the entry. Apache 2.0 is permissive: no copyleft on derivative works.

The non-agents packages (`-core`, `-react`, `-vue`, `-i18n`) were also previously AGPL through `@valent/docx-js-editor`; they're now Apache 2.0 as well.

## Vue users

Vue 3 is a published adapter in 1.0; it didn't exist in 0.x. If you were embedding the editor in a Vue app via the React adapter + a wrapper, drop the wrapper and use `@valent/docx-editor-vue` directly. The surface mirrors React (same component name, hooks-as-composables, same plugin contract). Cross-adapter parity is enforced by `scripts/parity/parity.contract.json` upstream. See the [Vue package page](/docs/1.x/vue).
