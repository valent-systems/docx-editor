...
Pulls in `@valent/docx-editor-core` and `@valent/docx-editor-i18n`. Add `@valent/docx-editor-agents` if you need the agent toolkit.
...
- `@valent/docx-editor-react`: `<DocxEditor>`, `DocxEditorRef`, `DocxEditorProps`, `EditorMode`, `createEmptyDocument` (re-exported from `-core` in 1.0.1)
- `@valent/docx-editor-react/ui`: `Toolbar`, `EditorToolbar`, `ResponsiveToolbar`, `TableToolbar`, `ToolbarButton`, `ToolbarGroup`, `ToolbarSeparator`, `ColorPicker`, font/color pickers
- `@valent/docx-editor-react/hooks`: `useHistory`, `useFindReplace`, `useAutoSave`, `useClipboard`, `useTableSelection`, `useWheelZoom`, `useDragAutoScroll`
- `@valent/docx-editor-react/dialogs`: hyperlink, find/replace, paste special, page setup, keyboard shortcuts
- `@valent/docx-editor-react/plugin-api`: `PluginHost`, plugin registration types
- `@valent/docx-editor-react/styles`: CSS
...
`@valent/docx-editor-react/ui` exports the toolbar pieces. `EditorToolbar` is the full toolbar used by the default editor UI; `Toolbar` is the formatting-button strip on its own; `ResponsiveToolbar` wraps either with adaptive layout. Compose your own from `ToolbarButton`, `ToolbarGroup`, and `ToolbarSeparator`:
...
import {
  ...
  ToolbarButton,
  ToolbarGroup,
} from '@valent/docx-editor-react/ui';
```
...
Wire AI tools to a live editor with `@valent/docx-editor-agents/react`:
