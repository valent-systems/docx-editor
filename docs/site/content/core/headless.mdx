---
title: 'Headless API'
description: 'Reference for @valent/docx-editor-core/headless: DOCX parse and serialize, document creation, content controls, templates, watermarks, DocumentAgent.'
category: 'Core'
---
...
```ts
import { parseDocx, repackDocx } from "@valent/docx-editor-core/headless";
...
```ts
import { getParagraphs, getParagraphText } from "@valent/docx-editor-core/headless";
...
| `createContentControl(doc, target, props?)` | Wrap a text span in a new control; returns `{ doc, control }` |
| `setContentControlContent(doc, filter, replacement, options?)` | Fill a rich/plain-text control with a string or `BlockContent[]` (`{ all: true }` fills every match) |
| `setContentControlValue(doc, filter, value, options?)` | Set a typed value: `{ kind: 'dropdown' \| 'checkbox' \| 'date', ... }` (`{ all: true }` sets every match) |
| `removeContentControl(doc, filter, options?)` | Delete a control, or unwrap with `keepContent: true` (`{ all: true }` removes every match) |
...
Errors are typed: `ContentControlNotFoundError`, `ContentControlLockedError`, `ContentControlTypeError`, `ContentControlBoundError`, `ContentControlKindError`, `ContentControlValueError`, `ContentControlCreateError`, `RepeatingSectionError`.
