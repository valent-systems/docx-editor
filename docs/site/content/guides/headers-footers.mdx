...
`getDocumentWatermark` and `setDocumentWatermark` from `@sqren/docx-editor-core/headless` read and write the same model:

```ts
import { parseDocx, setDocumentWatermark, repackDocx } from "@sqren/docx-editor-core/headless";
