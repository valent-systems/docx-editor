...
`getDocumentWatermark` and `setDocumentWatermark` from `@valent/docx-editor-core/headless` read and write the same model:

```ts
import { parseDocx, setDocumentWatermark, repackDocx } from "@valent/docx-editor-core/headless";
