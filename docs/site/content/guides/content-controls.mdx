...
By default the API searches the main document body. Pass `scope: 'all'` to also search headers and footers:
...
`scope: 'all'` needs a full `Document` (a bare `DocumentBody` has no header/footer parts), and a result's `location` tells you which part it lives in. When filling a control, the value inherits the surrounding formatting so it matches the text it replaces.

The live-editor `DocxEditorRef` methods (`getContentControls` / `setContentControlContent` / `removeContentControl`) cover the body only — they take no `scope`, so header and footer controls are addressable from the headless API.
