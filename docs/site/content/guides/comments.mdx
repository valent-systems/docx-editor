...
A `Comment` (exported from `@sqren/docx-editor-core`) is:
...
Find anchor paragraphs with `ref.current?.findInDocument(query)`, which returns matches with their `paraId`, then scroll with `scrollToParaId`.
