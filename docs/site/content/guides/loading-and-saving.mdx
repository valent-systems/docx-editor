...
If you already hold a parsed `Document` tree from `@valent/docx-editor-core`, pass it through the `document` prop instead and skip the parser.
...
For local crash recovery rather than server persistence, the React adapter ships `useAutoSave` in `@valent/docx-editor-react/hooks`: it persists the parsed `Document` to localStorage on an interval and exposes recovery data (`hasRecoveryData`, `acceptRecovery`) after a reload.
